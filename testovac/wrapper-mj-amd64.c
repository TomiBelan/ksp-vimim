 ...
 *      Note: for amd64 syscalls see http://www.milw0rm.com/papers/110
 */
  ...
  if (!proc_status_fd)
    {
      ...
      // if (proc_status_fd < 0) die("open(%s): %m", buf);
      // verime tomu ze nie su problemy s pravami -- ak neni, uz skoncil
      return -47;
    }
  lseek(proc_status_fd, 0, SEEK_SET);
  if ((c = read(proc_status_fd, buf, sizeof(buf)-1)) < 0) {
    // race condition: medzi casom skoncil?
    // skusime este raz otvorit
    sprintf(buf, "/proc/%d/stat", (int) box_pid);
    proc_status_fd = open(buf, O_RDONLY);
    if (proc_status_fd < 0) {
      // fprintf(stderr,"naozaj uz skoncil\n");
      return -47;
    }
...
valid_syscall(struct user *u)
{
  switch (u->regs.orig_rax)
    ...
    // case __NR_stat64:
    // case __NR_lstat64:
      // valid_filename((long long)u->regs.rbx);
      valid_filename((long long)u->regs.rdi);
      return 1;
    case __NR_exit:
    case __NR_read:
    case __NR_write:
    ...
    case __NR_times:
      return allow_times;
    case __NR_kill:
      if (u->regs.rdi == box_pid)
	die("Commited suicide by signal %d.", (int)u->regs.rsi);
      return 0;
    default:
      return 0;
    }
...
check_timeout(void)
{
  ...
  if (proc_msec > timeout + 2000 || proc_msec > 2*timeout || wall_msec > 4*timeout) {
    fprintf(stderr,"\nTime: %lld\nExitCode: 0\nErrors: TLE\n",last_elapsed_proc);
    box_exit();
  }
}
  ...
  for(;;)
      ...
      if (WIFSTOPPED(stat))
	{
	  ...
	  if (sig == SIGTRAP)
		...
		{
		  tolog(">> Syscall %3ld (%08lx,%08lx,%08lx) ", u.regs.orig_rax, u.regs.rbx, u.regs.rcx, u.regs.rdx);
		  syscall_count++;
		  if (!valid_syscall(&u))
		    {
		      /*
		       ...
		       */
		      unsigned int sys = u.regs.orig_rax;
		      u.regs.orig_rax = 0xffffffff;
